...
class OP_Woo{
		...
		if ( false !== $cached_data ) {
                ...
                switch ($type)
                {
                    ...
                    case 'variable':
                        if($post_type == 'product')
                        {
                            ...
                            $regular_price_list = $variations_result['regular_price_list'];
                            $qty_list = $variations_result['qty_list'];
                            $stock_manager = $product->get_manage_stock();
                            $qty = 0;
                            if($stock_manager && $warehouse_id == 0)
                            {
                                $qty = $product->get_stock_quantity();
                            }else{
            ...
            if($tmp_parent_id = wp_get_post_parent_id($product_id))
            {
                if($tmp_parent_id != $parent_id)
                {
                    ...
                    if($parent_post && $post_type == 'product_variation')
                    {
    ...
    public function get_parent_product_formatted_data($parent_post,$warehouse_id ,$ignore_variable = false,$is_search = false){
        ...
        if ( false !== $cached_data )
        {
            ...
            $variations = isset($tmp_v['variations']) ? $tmp_v['variations'] : array();
            $child_products = array();
            $data = array(
                'id' => $product_id,
                'barcode' => strtolower(trim($this->_core->getBarcode($product_id))),
